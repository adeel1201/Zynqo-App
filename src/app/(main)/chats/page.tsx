...
export default function ChatsPage() {
  ...
  return (
    <div className="flex flex-col animate-fade-in pb-20 min-h-screen bg-background">
      <header className="sticky top-0 z-40 w-full glass-morphism safe-top px-4 h- flex items-center justify-between">
      ...
      <div className="flex flex-col divide-y divide-border">
        {chatsLoading? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text- font-bold uppercase tracking-widest text-muted-foreground">Syncing Messages...</p>
          </div>
            ...
            return (
              ...
              >
                <div className="relative">
                  <Avatar className="w-14 h-14 border-border shadow-sm">
                    <AvatarImage src={chatPhoto} />
                    <AvatarFallback className={isGroup? "bg-primary/20 text-primary" : ""}>
                      {isGroup? <Users size={24} /> : chatName[0] || 'U'}
                    </AvatarFallback>
                  </Avatar>
                  {isOnline && (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    ...
                    <span className="text- text-muted-foreground font-bold uppercase tracking-widest shrink-0">
                      {timestamp}
                    </span>
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    <p className={`text-xs truncate leading-relaxed max-w-[85%] ${isUnread? 'text-foreground font-medium' : 'text-muted-foreground'}`}>
                      {isTyping? (
                        ...
                        <>
                          {chat.lastMessage?.senderId === user?.uid && <span className="text-primary/70 font-bold mr-1 text-">YOU:</span>}
                          {lastMsg}
                        </>
                      )}
                    </p>
                    {isUnread && (
                      <Badge className="bg-primary text- h-4 px-1.5 min-w-[1rem] flex items-center justify-center rounded-full">
                        NEW
                      </Badge>
                    )}
                  </div>
      ...
      {!isSearching && (
        <Dialog open={isNewChatOpen} onOpenChange={(open) => {
          ...
          <DialogContent className="bg-card border-border text-foreground rounded- max-w-[90vw] sm:max-w-[400px] p-0 overflow-hidden">
            <DialogHeader className="p-6 pb-0">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  className="rounded-xl border-primary/20 text-primary h-8 px-3 text- font-bold uppercase tracking-wider"
                >
            ...
            <div className="max-h-[50vh] overflow-y-auto no-scrollbar px-2 pb-6">
              <p className="text- font-bold uppercase tracking-widest text-muted-foreground ml-4 mb-2 opacity-50">
                    ...
                    >
                      <div className="relative">
                        <Avatar className="h-10 w-10 border-border">
                          ...
                          <AvatarFallback className="bg-primary/10 text-primary">{u.displayName?.[0] || 'U'}</AvatarFallback>
                        </Avatar>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text- text-muted-foreground font-medium tracking-widest uppercase truncate">@{u.username}</p>
                      </div>
