  ...
  return (
    <div className="flex flex-col animate-fade-in bg-background min-h-screen pb-24">
      ...
      <div className="p-4 flex-col gap-6">
        {/* Create Link */}
        <div className="flex items-center gap-4 bg-primary/5 p-5 rounded- border-primary/10 shadow-sm">
          ...
          <div className="flex-1">
            ...
            <p className="text- text-muted-foreground uppercase font-bold tracking-widest mt-1 opacity-60">Share a link for your Zynqo call</p>
          </div>
        </div>

        {/* Call History */}
        <div className="flex flex-col gap-4">
          <h4 className="text- font-bold uppercase tracking-[0.2em] text-muted-foreground ml-2">Recent Calls</h4>

          {loading? (
            <div className="flex flex-col items-center justify-center py-12 gap-4">
              <Loader2 className="animate-spin text-primary" size={24} />
              <p className="text- font-bold uppercase tracking-widest text-muted-foreground opacity-50">Syncing History...</p>
            </div>
          ) : calls.length > 0? (
            <div className="flex flex-col divide-y divide-border bg-card rounded- border-border overflow-hidden shadow-sm">
              {calls.map((call: any) => {
                ...
                return (
                  <div key={call.id} className="flex items-center gap-4 p-4 hover:bg-muted/30 transition-colors">
                    <Avatar className="w-12 h-12 border-border">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-1">
                        ...
                        <span className="text- text-muted-foreground font-bold uppercase tracking-wider">
                          {formatDistanceToNow(date, { addSuffix: true })}
                        </span>
                        {call.duration > 0 && (
                          <span className="text- text-muted-foreground/50 ml-1">• {Math.floor(call.duration / 60)}m {call.duration % 60}s</span>
                        )}
                      </div>
                    </div>
