...
import { ChevronLeft, X, MoreVertical, Loader2, Play, Pause } from 'lucide-react';
...
export default function StatusViewPage() {
  ...
  // Mark as viewed
  useEffect(() => {
    if (status && user && db && !status.viewers?.includes(user.uid)) {
  ...
  // Progress timer
  useEffect(() => {
    if (loading || !status || isPaused) return;
  ...
  }, [loading, status, isPaused, router]);
